 ...
 */
function Token (token, clientId, localCreationTime) {
  this.token = token;
  ...
  if (token) {
    try {
      ...
      if (this.content && this.content.iat && localCreationTime) {
          this.timeSkew = localCreationTime - (this.content.iat * 1000);
      }
    } catch (err) {
      this.content = {
        exp: 0
...
Token.prototype.isExpired = function isExpired () {
  let expireIn = this.content.exp * 1000 - Date.now();
  if (this.timeSkew) {
    expireIn += this.timeSkew;
  }
  return (expireIn < 0);
};
