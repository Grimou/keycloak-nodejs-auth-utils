...
GrantManager.prototype.createGrant = function createGrant (rawData) {
  ...
  let localCreationTime;
  let updatedRaw;
  if (grantData.local_creation_time) {
      localCreationTime = grantData.local_creation_time;
  } else {
      localCreationTime = Date.now();
      updatedRaw = rawData.substring(0, rawData.length-1) + ',"local_creation_time":' + localCreationTime + '}';
  }

  return this.validateGrant(new Grant({
    access_token: (grantData.access_token ? new Token(grantData.access_token, this.clientId, localCreationTime) : undefined),
    refresh_token: (grantData.refresh_token ? new Token(grantData.refresh_token, undefined, localCreationTime) : undefined),
    id_token: (grantData.id_token ? new Token(grantData.id_token, undefined, localCreationTime) : undefined),
    expires_in: grantData.expires_in,
    token_type: grantData.token_type,
    __raw: (updatedRaw ? updatedRaw : rawData),
